<template>
  <tr>
    ...
    <td class="flex gap-2 text-left md:text-center md:table-cell">
      <div class="inline-block w-1/3 md:hidden font-bold bg-table-header text-white">Athletic Div</div>
      <div>{{ school.division }}</div>
    </td>
    <td class="flex gap-2 text-left md:table-cell">
      <div class="inline-block w-1/3 md:hidden font-bold bg-table-header text-white">Conference</div>
      <div>{{ school.conference }}</div>
    </td>
    <td class="flex gap-2 items-center text-left md:text-center md:items-baseline md:table-cell">
    ...
    <td class="flex gap-2 items-center text-left md:text-center md:table-cell">
      ...
      <div>{{ getRange(school.sat.reading.min, school.sat.reading.max) }}</div>
    </td>
    <td class="flex gap-2 items-center text-left md:text-center md:table-cell">
      <div class="inline-block w-1/3 md:hidden font-bold bg-table-header text-white">
        SAT Math***<br />
      ...
      <div>{{ getRange(school.sat.math.min, school.sat.math.max) }}</div>
    </td>
    <td class="flex gap-2 items-center text-left md:text-center md:table-cell">
      <div class="inline-block w-1/3 md:hidden font-bold bg-table-header text-white">
        ACT Composite***<br />
      ...
      <div>{{ getRange(school.act.min, school.act.max) }}</div>
    </td>
  </tr>
</template>
